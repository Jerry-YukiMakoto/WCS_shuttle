...
namespace Mirle.MPLC.DataBlocks.DeviceRange
{
    public class DDeviceRange : ITypeDeviceRange
    {
        ...
        public DDeviceRange(string startAddress, string endAddress)
        {
            ...
            try
            {
                _startOffset = int.Parse(startAddress.Substring(1, startAddress.Length - 1));
                _endOffset = int.Parse(endAddress.Substring(1, endAddress.Length - 1));
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"{ex}");
