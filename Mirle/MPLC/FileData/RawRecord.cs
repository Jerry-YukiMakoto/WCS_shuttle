...
namespace Mirle.MPLC.FileData
{
    ...
    {
        private readonly object _cachedLock = new object();
        private readonly List<byte[]> _cachedRawData = new List<byte[]>();

        private string _rawString;

        public bool IsCached { get; private set; }

        public RawRecord(string rawString)
        {
            _rawString = rawString;
        }

        public byte[] GetBlockByIndex(int blockColumnIndex)
        {
            lock (_cachedLock)
            {
                try
                {
                    if (IsCached)
                    {
                        if (blockColumnIndex >= 0 && blockColumnIndex < _cachedRawData.Count)
                        {
                            return _cachedRawData[blockColumnIndex];
                        }
                    }
                    else
                    {
                        return ConvertRawStringToBlockByteArray(_rawString.Split('|')[blockColumnIndex]);
                    }
                }
                catch (Exception ex)
        ...
        public void CreateCache()
        {
            lock (_cachedLock)
            {
                try
                {
                    ...
                    string[] strList = _rawString.Split('|');
                    foreach (string strTemp in strList)
                    {
                        _cachedRawData.Add(ConvertRawStringToBlockByteArray(strTemp));
                    }
                    IsCached = true;
        ...
        protected virtual byte[] ConvertRawStringToBlockByteArray(string strTemp)
        {
            byte[] newByteArray = new byte[strTemp.Length / 2];
            for (int i = 0; i < strTemp.Length / 2; i++)
            {
                newByteArray[i] = ToByteLookup(strTemp.Substring(i * 2, 2));
            }
            return newByteArray;
        }
        private readonly Dictionary<string, byte> ByteLookup = new Dictionary<string, byte>()
        {
        ...
        private byte ToByteLookup(string hexNumber)
        {
