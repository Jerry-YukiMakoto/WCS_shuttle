...
namespace Mirle.DB.Fun
{
    public class clsEQ_Alarm
    {
        ...
        public bool FunUpdEQ_Alarm(int cvBuffer, string alarm, clsEnum.AlarmSts alarmSts, SqlServer db)
        {
            try
            {
                var now = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff");
                var update = $"UPDATE UnitStsLog SET CLRDT='{now}', TotalSecs=datediff(SECOND,STRDT, '{now}') , AlarmSts = '{(int)alarmSts}'" +
        ...
        public bool FunInsEQ_Alarm(int cvBuffer, string alarm, clsEnum.AlarmSts alarmSts, SqlServer db)
        {
            try
            {
